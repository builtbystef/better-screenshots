...
features/studio/ the Studio
  ...
  hooks/         one hook per Preview or Inspector DOM concern
...
| `features/studio/composition/catalog.ts`              | Catalog data and `catalogSolidFor`, `catalogGradientFor`, `aspectPresetFor`                                                                                                                                     | The eight solid Backgrounds, six gradient Backgrounds, default Background, and nine Aspect presets                                                                                                                                                         |
...
| `features/studio/composition/saved-composition.ts`    | `SavedComposition`, `serializeSavedComposition`, `parseSavedComposition`, `applySavedComposition`                                                                                                               | The saved shape and its version; applying goes through the Session writers, so stored values re-validate like live edits                                                                                                                                   |
| `features/studio/platform/paint.ts`                   | `createPainter(...) -> Painter`, `paintScreenshot`, `paintBrowserWindow`, and `PAINT_SCALE`                                                                                                                     | Canvas 2D drawing, image decoding, and the decoded-bitmap cache that keeps a repaint from re-decoding                                                                                                                                                      |
...
| `features/studio/platform/notify.ts`                  | `notifyRefusal(line)`                                                                                                                                                                                           | Sonner: every refusal message reaches the user as one error toast, keyed on its own text so a repeat replaces rather than stacks                                                                                                                           |
| `features/studio/platform/saved-composition-store.ts` | `createSavedCompositionStore() -> SavedCompositionStore`                                                                                                                                                        | localStorage and its key; storage failures are translated to `"unavailable"`, and a stored value that does not parse loads as `"none"`                                                                                                                     |
...
| `features/studio/studio-page.tsx`                     | `StudioPage()`                                                                                                                                                                                                  | The Session boot: it creates the Session with the IndexedDB adapter, restores the Saved composition, subscribes the saver, and hands the Session to `StudioShell`                                                                                          |
| `features/studio/index.ts`                            | `StudioPage`                                                                                                                                                                                                    | Every other module in the feature; the route imports this file and no deeper                                                                                                                                                                               |
| `features/studio/components/studio-shell.tsx`         | `StudioShell({ session })`                                                                                                                                                                                      | The Studio shell layout, the version subscription, the `BrandMark` wordmark icon, and the five `InspectorSection` groups                                                                                                                                   |
...
| `features/studio/components/inspector/`               | One Inspector section per file: `FrameInspector`, `BackgroundInspector`, `PlacementInspector`, `WindowInspector`, `EffectsInspector`, `ActionsInspector`                                                        | The section markup, the shared `KnobRow`, `PositionRow`, and `BorderColorRow`, the `styles.ts` class variants, and `gradientCss`                                                                                                                           |
| `features/studio/hooks/`                              | One hook per Preview or Inspector DOM concern: `usePreviewCanvas`, `useFileDrop`, `useScreenshotDrag`, `useExport`, `usePlaceScreenshot`                                                                        | Canvas mounting, the window drag and paste listeners, pointer drag state, the export download, and the place-a-Screenshot call                                                                                                                             |
| `routes/index.tsx`                                    | TanStack Router's `/` route with `StudioRoute`                                                                                                                                                                  | Nothing: it declares the route and renders `StudioPage`                                                                                                                                                                                                    |
| `routes/__root.tsx`                                   | TanStack Router's root route                                                                                                                                                                                    | The document shell, the head tags including the two favicon links, the `Toaster` mount, and the colour-scheme boot script                                                                                                                                  |
...
| `components/ui/`                                      | One shadcn presentation primitive per file: `Button`, `Input`, `Label`, `Slider`, `Toggle`, `ToggleGroup`, `Separator`, `Sidebar`, `Toaster`, and the `Sheet`, `Tooltip`, and `Skeleton` that `Sidebar` imports | Base UI behaviour and the Tailwind class strings that style it; each file is vendored from the `base-nova` registry that `components.json` names, and is edited only to keep the formatter happy, plus the two recorded exceptions: `Toaster` and `Slider` |
...
features/studio/platform        -> features/studio/composition (types, and the Saved composition parser)
...
The `studio-well` surface backs the whole page rather than the Preview alone, so the Inspector floats on it. The Inspector is the shadcn `Sidebar` primitive: `side="right"`, `collapsible="none"`, and the floating look — the inset margin, the rounded corners, the shadow, and the `ring-sidebar-border` outline — stated as classes on the `Sidebar` itself, because the vendored component only applies `variant="floating"` on the collapsible branch. Below the `md` breakpoint the shell stacks instead of shrinking: the Preview keeps the top of the screen and the Inspector becomes a full-width bottom panel capped at `46svh` that scrolls inside its own box, so a phone never scrolls the page sideways. It is still the Inspector in the glossary's sense; `Sidebar` is the primitive that renders it. Its header carries the brand icon inline as `BrandMark` beside the name as live text, so both take `currentColor` and Poppins and neither needs a file per colour scheme. The favicon is the same icon as drawn — black stroke, no scheme swap — served twice: `favicon.svg`, and `favicon.ico` behind it for whatever will not take an SVG. Its footer carries Replace and Export, which is where the Studio's top bar went: the shell is now the Preview and the Inspector, and nothing else. `Card` left `components/ui/` with the top bar, since the Inspector sections are `SidebarGroup`s now.

Every refusal message is a toast. `messages.ts` still owns the copy and still answers `null` for `ok`; `platform/notify.ts` is the one place that turns a non-null line into `toast.error`. Before, each caller held the line in its own `useState` and painted it as muted text — over the Composition in the Preview, where the Background is the user's to pick and no text colour is safe, and under the control in two Inspector sections. Now no component holds outcome state at all. Sonner's toast id is the message text, so a refusal that repeats replaces its toast instead of stacking. The vendored `Toaster` is edited twice: `next-themes` is dropped, since `schemeBootScript` and Sonner's `system` read the same media query, and `font-sans` takes the important flag to beat the `font-family` Sonner injects at runtime. The vendored `Slider` is edited once: `aria-label` is forwarded to each thumb's `getAriaLabel`, because the element a screen reader lands on is the thumb's `<input type="range">`, and on the root div the label names nothing focusable. Poppins is the app face, self-hosted from `@fontsource/poppins` as the latin subset in four weights, and set once as `--font-sans`.
...
- `PAINT_SCALE = 2`: the Export canvas is twice the Frame dimensions. The Preview runs the same paint passes at the scale that fills its box, capped at `PAINT_SCALE`, so it never spends more pixels than the Export would. The constant is not part of the Composition.
...
Preview and Export share the Canvas 2D draw path required by ADR 0001. Painting clips to the Frame and runs the same paint passes at a requested scale: the Export always paints at `PAINT_SCALE` and serializes with `toBlob`; the Preview paints into its one long-lived canvas at the scale that fills its box, capped at `PAINT_SCALE`. The painter is created once per Session and caches decoded bitmaps, so a slider drag repaints without re-decoding the Screenshot or re-reading the Uploaded background.

### Saved composition persistence

`SavedCompositionStore` is the persistence port for the Saved composition; `createSavedCompositionStore` is the localStorage adapter. The boot loads it once, applies it through the Session writers — so a stored value that no longer validates falls back to that field's default — and saves on every commit. The Screenshot is never part of it.
