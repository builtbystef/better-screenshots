...
| `apps/web/src/chrome.ts`           | Pure helpers for messages, parsing, Catalog matching, drag geometry, and colour scheme | Refusal copy, input syntax, chip matching, Position calculations, and the light/dark rule                                                                                                                   |
...
```text
routes/index.tsx -> session, catalog, chrome, indexed-db-store
...
| **Target** | `scheme.ts`           | The single light/dark rule and its boot script                              | Colour-scheme DOM integration                         |
